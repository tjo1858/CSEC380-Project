...
def test_act4():
    ...
    data = {'username': 'admin', 'password': 'admin'}
    r = session.post('http://webapp:5000/login', data=data)
    assert 'RITube Video System' in r.text
    t = session.post("http://webapp:5000/homepage", files={"file": open("Activity 4/SampleVideo_1280x720_1mb.mp4", "rb")})
    print(t.text)
    assert session.get("http://webapp:5000/videos/SampleVideo_1280x720_1mb.mp4").status_code == 200
